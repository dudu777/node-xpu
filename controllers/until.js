...
var User = require('../models/User').User  //引入User数据库模型
...
const obj={
    ...
    checkToken:function(req,res,next) {  //检查token
        ...
        if(!t) {   //如果请求头中不存在token，返回"no token"信息
            res.send(obj.setResult(500,'no token',t))
            return
        }
        var token = jwt.decode(t,'hhdj') //token存在，进行解密
        if(!token) {
            res.send(obj.setResult(500,'token error',err))  //解密不成功
            return
        }
        //解密成功，查询
        User.find({where:{id:token.id}}).then(r=>{
          ...
          //将在数据里边查询的信息，保存到req. TOKEN_USER里边
            ...
            res.send(obj.setResult(0,'token验证失败',r))
        })
    },
    /* 接口统一返回格式 */
